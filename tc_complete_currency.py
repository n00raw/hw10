...
# Task 3 : implement the `convert` tool; add its tool definition and write the function body.
...
class CurrencyTools:
    ...
    def convert(self, amount: float, base: str, quote: str) -> Dict[str, Any]:
        ...
        """
        if f"{base}->{quote}" not in RATE_TABLE:
            return {"error": f"No rate for {base}->{quote}"}

        rate = RATE_TABLE[f"{base}->{quote}"]
        result = amount*rate
...
# FINAL: The result of converting 100 USD to THB is 3500.0 THB.
# === INTERMEDIATE (turn 1) ===
# name: convert
